...
在AW06中，基于MySQL中保存的Amazon商品数据作为数据库。

对原有的接口实现做了一些修改，通过分页的方式以适应大规模数据。

通过redis缓存减少了对MySQL服务器的访问，缓存主要集中在数据库相关接口中。

在当前的设置下，MySQL服务器中搜索一个商品，或者获取一个商品需要的时间约为100ms量级。（数据量～400000条数据）

最小化了此应用使用的MySQL账户权限——仅需要SELECT权限。

### aw06运行需要

在本机6379端口运行的redis服务器。

在本机3306端口运行的MySQL服务器，其中要有一个名为amazon-products的数据库。

MySQL服务器上要有一个支持SELECT权限的账户products，密码可以在application.properties文件中配置。

程序运行所需要的表可以在aw06项目运行中构建。并且除非数据需要更新，否则我们没有必要多次运行这个批处理任务。
